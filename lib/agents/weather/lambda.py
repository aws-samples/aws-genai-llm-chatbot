...
def get_current_weather(place: Annotated[str, Query(description="the name of the place")]) -> Weather:
    resp = loc_client.search_place_index_for_text(IndexName=os.environ.get("LOCATION_INDEX", os.environ.get("PLACE_INDEX", "Test")), Text=place)
...
@app.get("/historical_weather", description="get the historical daily mean temperature and precipitation for a given place for a range of dates")
def get_historical_weather(place: Annotated[str, Query(description="the name of the place")], 
    ...
    resp = requests.get(q.format(lat=lat, lon=lon, fromDate=fromDate, toDate=toDate)).json()
    ...
    hourly_values = hourly_values.resample("D").agg({"temperature_2m": "mean", "precipitation": "sum"})
    return [ Weather(time=str(hourly_values.iloc[i].name).split(" ")[0],
                   temperature=hourly_values.iloc[i][0], 
                   precipitation=hourly_values.iloc[i][1]) for i in range(hourly_values.shape[0])]
