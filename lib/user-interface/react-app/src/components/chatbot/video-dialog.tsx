...
export default function VideoDialog(props: VideoDialogProps) {
  ...
  return (
    <Modal
      ...
      header="Add a video to your message"
    >
      <Form>
        <SpaceBetween size="m">
          <FormField
            ...
            description="You can upload a single video to be used for this conversation."
          >
            <FileUpload
              onChange={({ detail }) => {
              ...
              showFileThumbnail
              tokenLimit={3}
              constraintText={`${ALLOWED_MIME_TYPES.join(", ")}. Max 10MB.`}
            />
          </FormField>
          {loading && (
            <>
              <div>
                <Spinner />
                <span style={{ marginLeft: "5px" }}>Adding video...</span>
              </div>
            </>
          )}
        </SpaceBetween>
