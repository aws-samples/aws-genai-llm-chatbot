...
def get_bedrock_client(service_name="bedrock", timeout=60):
    ...
    config_data = {"service_name": service_name, "config": config}
...
def get_bedrock_agent_client(timeout=30):
    """
    Get a boto3 client for bedrock-agent-runtime, with cross-account support
    if configured

    Args:
        timeout (int, optional): The timeout in seconds for API calls.
            Defaults to 30.
...
def list_agents(region: Optional[str] = None) -> list[dict[str, Any]]:
    ...
    Args:
        region (str, optional): The AWS region to list agents from.
            Defaults to None (uses BEDROCK_REGION from environment).
    ...
    try:
        ...
        for agent_summary in response.get("agentSummaries", []):
            ...
            try:
                ...
                logger.error(
                    f"Error listing aliases for agent {agent_id}: {str(alias_error)}"
                )
                aliases = []
            ...
            try:
                ...
                logger.error(
                    f"Error listing versions for agent {agent_id}: {str(version_error)}"
                )
                versions = []
...
def invoke_agent_by_id(
    ...
    Args:
        ...
        agent_alias_id (str, optional): The alias ID of the agent to invoke.
            Defaults to None.
        agent_version (str, optional): The version of the agent to invoke.
            Defaults to "DRAFT".
        session_id (str, optional): The session ID to use for the agent.
            Defaults to None (generates a new session ID).
    ...
    try:
        ...
        if not agent_alias_id and agent_version == "DRAFT":
            try:
                ...
                logger.warning(
                    f"Error getting latest agent alias, falling back to DRAFT: "
                    f"{str(alias_error)}"
                )
...
def extract_metadata_from_response(response_stream: Dict[str, Any]) -> Dict[str, Any]:
    ...
    """
    metadata = {"trace": {}, "sessionAttributes": {}}
...
def process_completion_value(completion_value: Any) -> str:
    ...
    if (
        hasattr(completion_value, "__class__")
        and completion_value.__class__.__name__ == "EventStream"
    ):
...
def extract_completion_from_response(response_stream: Any, prompt: str) -> str:
    ...
    if not completion:
        ...
        response_attrs = (
            dir(response_stream)
            if hasattr(response_stream, "__dict__")
            else "No __dict__"
        )
        logger.error(f"Response stream attributes: {response_attrs}")
        ...
        if isinstance(response_stream, dict):
            if "completion" in response_stream:
                ...
                logger.info(
                    "Found ResponseMetadata in response, this might be a raw "
                    "AWS SDK response"
                )
                if "HTTPStatusCode" in response_stream.get("ResponseMetadata", {}):
                    ...
                    if status_code != 200:
                        completion = (
                            f"The request to the Bedrock agent failed with "
        ...
        if not completion:
            completion = (
                "I'm sorry, but I encountered an issue while processing "
                "your request. Please try again with a simpler query "
                "or without file attachments."
            )
...
def invoke_agent(session_id, prompt, enable_trace=True, timeout=60):
    ...
    Raises:
        CommonError: If Bedrock Agent is not enabled or if there's an error
            invoking the agent
    """
