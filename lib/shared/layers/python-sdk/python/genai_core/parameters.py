...
def get_origin_verify_header_value():
    origin_verify_header_value = parameters.get_secret(
        X_ORIGIN_VERIFY_SECRET_ARN, transform="json", max_age=60)["headerValue"]
...
def get_sagemaker_llms():
    return parameters.get_parameter(
        LARGE_LANGUAGE_MODELS_PARAMETER_NAME, transform="json", max_age=60)
