...
export interface WebCrawlerWorkflowProps {
...
export class WebCrawlerWorkflow extends Construct {
  public readonly stateMachine: sfn.StateMachine;

  constructor(scope: Construct, id: string, props: WebCrawlerWorkflowProps) {
    super(scope, id);
    ...
    const logGroup = new logs.LogGroup(this, "WebCrawlerSMLogGroup", {
      removalPolicy: RemovalPolicy.DESTROY,
    });

    const workflow = setProcessing.next(webCrawlerJob).next(setProcessed);
    const stateMachine = new sfn.StateMachine(this, "WebCrawlerStateMachine", {
      definitionBody: sfn.DefinitionBody.fromChainable(workflow),
      // timeout: cdk.Duration.hours(12),
      comment: "Web Crawler Workflow",
      tracingEnabled: true,
      logs: {
