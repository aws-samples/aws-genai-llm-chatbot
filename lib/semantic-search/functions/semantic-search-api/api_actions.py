...
def semantic_search(event):
    ...
    try:
        ...
        logger.debug(
            f"Querying database for nearest neighbors by '{operator}'")

        if operator == "cosine":  # cosine distance
            cur.execute("SELECT id, url, content, content_embeddings <=> %s AS metric FROM documents ORDER BY metric LIMIT %s;", [
        ...
        logger.debug(
            "Querying database by keyword match")
        cur.execute(
