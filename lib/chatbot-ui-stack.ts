...
export class ChatBotUIStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    ...
    const exportsAsset = s3deploy.Source.jsonData('aws-exports.json', {
      ...
      aws_user_pools_id: ssm.StringParameter.fromStringParameterName(this, 'UserPoolId', '/chatbot/cognito/user-pool-id').stringValue,
      aws_user_pools_web_client_id: ssm.StringParameter.fromStringParameterName(this, 'UserPoolClientId', '/chatbot/cognito/user-pool-client-id').stringValue,
      identityPoolId: ssm.StringParameter.fromStringParameterName(this, 'IdentityPoolId', '/chatbot/cognito/identity-pool-id').stringValue,
      client: {
        name: 'lambda',
        send_endpoint: ssm.StringParameter.fromStringParameterName(this, 'ChatBotEndpointSend', '/chatbot/endpoints/send-message').stringValue,
        action_endpoint: ssm.StringParameter.fromStringParameterName(this, 'ChatBotEndpointAction', '/chatbot/endpoints/chat-action').stringValue,
      },
    });
    ...
    const asset = s3deploy.Source.asset(appPath, {
      bundling: {
        ...
        command: ['sh', '-c', ['npm --cache /tmp/.npm install', `npm --cache /tmp/.npm run build`, 'cp -aur /asset-input/build/* /asset-output/'].join(' && ')],
        local: {
