...
def _wait_for_session(client: AppSyncClient, session_id: str, expected_messages: int = 2, timeout: int = 30):
    ...
    while time.time() - start_time < timeout:
        try:
            ...
            pass
        time.sleep(1)
    raise TimeoutError(f"Session {session_id} did not reach {expected_messages} messages within {timeout}s")


def test_agent_conversation_single_turn(client: AppSyncClient, available_agent, agent_session_id):
...
def test_agent_conversation_multi_turn(client: AppSyncClient, available_agent, agent_session_id):
    ...
    request1 = {
        ...
        "data": {
            ...
            "modelName": "anthropic.claude-3-haiku-20240307-v1:0",
            "provider": "bedrock",
            "modelKwargs": {"streaming": False, "maxTokens": 2000, "temperature": 0.7, "topP": 0.9}
        }
    }
...
def test_agent_conversation_session_persistence(client: AppSyncClient, available_agent, agent_session_id):
